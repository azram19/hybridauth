...
/**
 * Hybrid_Providers_YahooOAuth - Yahoo provider adapter based on OAuth1 protocol
 */
class Hybrid_Providers_YahooOAuth extends Hybrid_Provider_Model_OAuth1
{
	...
	*/ 
	/*function setUserStatus( $status )
	{
		...
		$parameters = array();
		$parameters['format']	= 'json';
		$parameters['status']	= array('status' => array( 'message' => $status ));
		var_dump(json_encode($parameters['status']));
		$response = $this->api->request('user/' . $userId . '/profile/status', 'PUT', $parameters);

		var_dump($response);

		if( ! $response->status || ! $response->status->message || $this->api->http_code != 200 )
		{
			throw new Exception( 'User status update failed! ' . $this->providerId . ' returned an error: ' . $this->errorMessageByStatus( $this->api->http_code ) );
		}
	}*/
}